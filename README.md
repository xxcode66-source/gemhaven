...
> v2.5 note: GemHaven v1 was a round-based parimutuel game ("Delves"), and v2.0 briefly carried a Megapot fee route ("the Deep Vein"). Both are gone from the live game: v2.5 is a zinc-style instant game against a house bankroll with a rolling Bonanza pot, powered purely by Inco Lightning. On top of v2.4 it rightsizes the Inco fee budget: exactly two operations per Dig carry a fee (the sealed pick and the draw), so the budget drops from 5–6 units to 3 (two plus headroom), cutting each player's per-Dig Inco cost ~40–50%. The house revenue layer (v2.3): a 1% protocol fee on every Dig (`withdrawFees`), edge harvesting above a protected bankroll floor (`skimProfit`), and an owner-adjustable per-Dig exposure cap (`payoutCapBps`). Earlier Base Sepolia deployments remain on chain as legacy artifacts; the live game is the v2.5 deployment below.
...
Wins are paid from the **bankroll**, so the house enforces a solvency cap: `maxStake() = maxPayout() × 100 / 3492`, where `maxPayout() = bankroll × payoutCapBps / 10000` is the per-Dig exposure cap (100% by default; mainnet operators should lower it, e.g. 500 = 5%, so a single win cannot gut the house). A Dig whose payout would exceed that line reverts, and the UI caps amounts at it with a message saying why. The bankroll is seeded at deploy (`BANKROLL_SEED_WEI`) and grows on net losing play.

**House revenue, precisely:** the protocol fee is certain — it accrues on every Dig regardless of outcome — while the ~2% net house edge (multiplier edge minus the recycled Bonanza cut) is statistical and accrues in the bankroll as growth. `skimProfit(bps, to)` harvests a share of the bankroll growth above `bankrollFloor` (set to the seed at deploy, and only ever raisable), so the seeded principal always stays behind player payouts. Intended operator rhythm: skim periodically and split between operations and a treasury that seeds future token liquidity — a policy commitment, not an on-chain mechanism.
...
### Live deployment — Base Sepolia (v2.5)
...
- **Solvency caps small bankrolls.** With the testnet bankroll of 0.03543 ETH, Picks above ~0.001 ETH revert by design. Mainnet operators would seed deeper (`BANKROLL_SEED_WEI` or live `fundBankroll` top-ups); the cap is a feature, not a bug — the contract can never owe more than it holds.
...
- **Deployment is fresh.** The v2.5 Base Sepolia deployment above exists and its reads are verified, but a full Dig (bet → decrypt → claim), an Even/Odd/All pass, and an auto loop have not yet been played end to end through the UI.
